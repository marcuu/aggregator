...
describe("calculateMonthlySurplus", () => {
  ...
  it("averages income minus spend over the 90-day window", () => {
    // £9,000 income and £4,500 spend over 3 months => £1,500/mo.
    const transactions = [
    ...
    expect(calculateMonthlySurplus(transactions, AS_OF)).toBe(150_000);
  });
  ...
  it("excludes internal transfers from both sides", () => {
    ...
    // Only the £3,000 salary counts: 3000 / 3 months = £1,000/mo.
    expect(calculateMonthlySurplus(transactions, AS_OF)).toBe(100_000);
  });

  it("returns a negative figure when spending exceeds income", () => {
    const transactions = [
    ...
    expect(calculateMonthlySurplus(transactions, AS_OF)).toBeLessThan(0);
  });
  ...
  it("ignores transactions older than the 90-day window", () => {
    ...
    // Only the in-window £3,000 counts: 3000 / 3 = £1,000/mo.
    expect(calculateMonthlySurplus(transactions, AS_OF)).toBe(100_000);
  });
});
